...
export default function AlgorithmsPage() {
  const [algos, setAlgos] = useState<Record<string, any>[]>([]);
  ...
  async function togglePublish(id: string, current: boolean) {
    await supabase.from('algorithms').update({ published: !current }).eq('id', id);
    fetchAlgos();
  }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold tracking-tight">Algorithm Archives</h1>
        <Link
          href="/algorithms/new"
      ...
      <div className="bg-white border text-sm border-gray-200 shadow-sm rounded-xl overflow-hidden">
        {loading ? (
          <div className="p-8 text-center text-gray-500">Loading algorithms...</div>
        ) : algos.length === 0 ? (
          <div className="p-8 text-center text-gray-500">No algorithms found.</div>
        ) : (
